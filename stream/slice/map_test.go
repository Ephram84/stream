...
func TestReducingToFloat(t *testing.T) {
	...
	result, err := FromMapWithSlices(transactionsMap).MapToFloat64(func(elem Transaction) (float64, error) { return elem.Amount, nil }).Reduce(accumulator.Sum).ToMap()
	assert.NoError(t, err)
...
func TestReduce(t *testing.T) {
	...
	sum, err := FromMapWithSlices(m).Reduce(accumulator.Sum).ToMap()
	assert.NoError(t, err)
	assert.Equal(t, map[string]float64{
		"positive": 25.0,
		"negative": -15.0,
	...
	avg, err := FromMapWithSlices(m).Reduce(accumulator.Avg).ToMap()
	assert.NoError(t, err)
	assert.Equal(t, map[string]float64{
		"positive": 12.5,
		"negative": -7.5,
	...
	min, err := FromMapWithSlices(m).Reduce(accumulator.Min).ToMap()
	assert.NoError(t, err)
	assert.Equal(t, map[string]float64{
		"positive": 10.0,
		"negative": -10.0,
	...
	max, err := FromMapWithSlices(m).Reduce(accumulator.Max).ToMap()
	assert.NoError(t, err)
	assert.Equal(t, map[string]float64{
		"positive": 15.0,
		"negative": -5.0,
