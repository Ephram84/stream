...
func TestErrorSequence(t *testing.T) {
	...
	result, err := FromSlice(numbers).MapToInt(common.StringToInt).Filter(isEven).ToSlice()
	assert.Error(t, err)
...
func TestDistinctSequence(t *testing.T) {
	result, err := FromSlice([]int{1, 2, 2, 3, 3, 3, 4, 4, 4, 4}).
		Distinct().
		ToSlice()
	assert.NoError(t, err)
...
func TestCountSequence(t *testing.T) {
	count, err := FromSlice([]int{1, 2, 3, 4, 5}).Count()
	assert.NoError(t, err)
	assert.Equal(t, 5, count)
}

func TestFirstSequence(t *testing.T) {
	result, err := FromSlice([]int{1, 2, 3, 4, 5}).First()
	assert.NoError(t, err)
...
func TestFirstReturnsNilSequence(t *testing.T) {
	result, err := FromSlice([]int{1, 2, 3, 4, 5}).Filter(func(elem int) (bool, error) {
		return elem > 5, nil
	}).First()
	assert.NoError(t, err)
	assert.Nil(t, result)
...
func TestFirstReturnsElseSequence(t *testing.T) {
	result, err := FromSlice([]int{}).First(42)
	assert.NoError(t, err)
	assert.NotNil(t, result)
...
func TestAnyMatchSequence(t *testing.T) {
	result, err := FromSlice([]int{1, 2, 3, 4, 5}).AnyMatch(func(elem int) (bool, error) {
		return elem == 3, nil
	})
	assert.NoError(t, err)
...
func TestAllMatchSequence(t *testing.T) {
	result, err := FromSlice([]int{2, 4, 6, 8}).AllMatch(func(elem int) (bool, error) {
		return elem%2 == 0, nil
	})
	assert.NoError(t, err)
	assert.True(t, result)
...
func TestAllMatchReturnsFalseSequence(t *testing.T) {
	result, err := FromSlice([]int{2, 4, 6, 7, 8}).AllMatch(func(elem int) (bool, error) {
		return elem%2 == 0, nil
	})
	assert.NoError(t, err)
	assert.False(t, result)
...
func TestNoneMatchSequence(t *testing.T) {
	result, err := FromSlice([]int{1, 3, 5, 7}).NoneMatch(func(elem int) (bool, error) {
		return elem%2 == 0, nil
	})
	assert.NoError(t, err)
	assert.True(t, result)
...
func TestNoneMatchReturnsFalseSequence(t *testing.T) {
	result, err := FromSlice([]int{1, 2, 3, 5, 7}).NoneMatch(func(elem int) (bool, error) {
		return elem%2 == 0, nil
	})
	assert.NoError(t, err)
	assert.False(t, result)
...
func TestReduceSumSequence(t *testing.T) {
	result, err := FromSlice([]int{1, 2, 3, 4, 5}).Reduce(0, accumulator.Sum[int]())
	assert.NoError(t, err)
	assert.Equal(t, 15, result)
}

func TestConcatSequences(t *testing.T) {
	result, err := FromSlice([]int{4, 5}).Concat(FromSlice([]int{6, 7, 8})).ToSlice()
	assert.NoError(t, err)
	assert.Equal(t, []int{4, 5, 6, 7, 8}, result)
}

func TestMapSequence(t *testing.T) {
	numbers := FromSlice([]int{1, 2, 3, 4, 5})
...
func TestGroupBySequence(t *testing.T) {
	numbers := FromSlice([]int{1, 2, 3, 4, 5, 6})
...
func TestFlatMapSeqWithInts(t *testing.T) {
	...
	result, err := FlatMapSeq(FromSlice(intSlice), func(elem []int) (*seq[int], error) {
		return FromSlice(elem), nil
	}).ToSlice()
	assert.NoError(t, err)
...
func TestZipSequences(t *testing.T) {
	result, err := ZipSeqs(FromSlice([]int{1, 2, 3}), FromSlice([]string{"a", "b", "c", "d"})).ToSlice()
	assert.NoError(t, err)
